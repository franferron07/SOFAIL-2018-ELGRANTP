...
int cargar_archivo_config(char* path) {
	...
	cpu.ip_safa = config_get_int_value(config, "IP_SAFA");
	cpu.puerto_safa = config_get_int_value(config, "PUERTO_SAFA");
	cpu.ip_diego = config_get_int_value(config, "IP_DIEGO");
	cpu.puerto_diego = config_get_int_value(config, "PUERTO_DIEGO");
	cpu.ip_fm9 = config_get_int_value(config, "IP_FM9");
...
int crear_log() {
	cpu_log = log_create("cpu.log", "El-Gran-TP-CPU", true, LOG_LEVEL_TRACE);
	if (cpu_log == NULL) {
...
void imprimir_config() {
	...
	log_info(cpu_log, "IP_SAFA: %d", cpu.ip_safa);
	log_info(cpu_log, "PUERTO_SAFA: %d", cpu.puerto_safa);
	log_info(cpu_log, "IP_DIEGO: %d", cpu.ip_diego);
	log_info(cpu_log, "PUERTO_DIEGO: %d", cpu.puerto_diego);
	log_info(cpu_log, "IP_FM9: %d", cpu.ip_fm9);
